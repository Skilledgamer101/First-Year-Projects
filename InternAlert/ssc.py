...
# Now just redirect to SSC jobs page
...
all = soup.select('td')
if all == None:
    message = "No SSC jobs exist in the search"
    ezgmail.send(recipient, 'No SSC jobs exist in the search', message)
    print("\n\nNo SSC jobs exist in the search\n\n")
    exit(0)
...
if message != '':
    ...
    ezgmail.send(recipient, 'No new SSC jobs till now (EOM)', 'Hopefully soon :)')
